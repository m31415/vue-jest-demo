...
src/
├── types/
│   ├── passing/          # 14 type files that work ✅
│   │   ├── conditional-single-prop.ts
│   │   ├── conditional-with-index.ts
│   │   ├── generic-conditional-single-prop.ts
│   │   ├── generic-intersection.ts
│   │   ├── generic-utility.ts
│   │   ├── generic.ts
│   │   ├── index-access-intersection.ts
│   │   ├── index-access-utility.ts
│   │   ├── index-access.ts
│   │   ├── intersection.ts
│   │   ├── mapped-single-prop.ts
│   │   ├── nested-conditional-prop.ts
│   │   ├── union-with-conditional.ts
│   │   └── utility.ts
│   └── failing/          # 9 type files that fail ❌
│       ├── conditional.ts
│       ├── generic-conditional.ts
│       ├── generic-index-access-simple.ts
│       ├── generic-index-intersection.ts
│       ├── generic-index-utility.ts
│       ├── generic-mapped.ts
│       ├── index-access-conditional.ts
│       ├── index-access-mapped.ts
│       └── mapped.ts
└── components/
    ├── passing/          # 14 tests that pass ✅
    │   ├── ConditionalSingleProp.vue
    │   ├── ConditionalWithIndex.vue
    │   ├── Generic.vue
    │   ├── GenericConditionalSingleProp.vue
    │   ├── GenericIntersection.vue
    │   ├── GenericUtility.vue
    │   ├── IndexAccess.vue
    │   ├── IndexAccessIntersection.vue
    │   ├── IndexAccessUtility.vue
    │   ├── Intersection.vue
    │   ├── MappedSingleProp.vue
    │   ├── NestedConditionalProp.vue
    │   ├── UnionWithConditional.vue
    │   └── Utility.vue
    └── failing/          # 9 tests that fail ❌
        ├── Conditional.vue
        ├── GenericConditional.vue
        ├── GenericIndexAccessSimple.vue
        ├── GenericIndexIntersection.vue
        ├── GenericIndexUtility.vue
        ├── GenericMapped.vue
        ├── IndexAccessConditional.vue
        ├── IndexAccessMapped.vue
        └── Mapped.vue
```

**Naming Convention:**
- Files named by features they test (e.g., `generic-utility.ts`)
- No "NoIndex" or "Only" suffixes - if a feature isn't present, it's not in the name
- Files with index access include "index" in the name (e.g., `generic-index-utility.ts`)
- Organized by outcome: `passing/` for working types, `failing/` for broken types
